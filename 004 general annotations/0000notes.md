...
`@SpringBootApplication` is the primary annotation used to mark the main class of a Spring Boot application. It's a **convenience annotation** that combines three other annotations, saving you from stacking multiple annotations on your main class.
...
A specialized form of `@Configuration`. It marks the class as a source of bean definitions for the application context — basically saying "this class can contain `@Bean` methods." Internally it's just `@Configuration` with extra meta-annotation info so tools can identify it as the Spring Boot entry point.

### 2. `@EnableAutoConfiguration`
This is the real "magic" of Spring Boot. It tells Spring Boot to automatically configure your application based on:
- The dependencies (JARs) on your classpath
- Beans you've already defined

For example, if `spring-boot-starter-web` is on the classpath, it auto-configures an embedded Tomcat server and Spring MVC. If it sees a `DataSource` bean, it may auto-configure JPA-related beans. This works via conditional configuration classes (`@ConditionalOnClass`, `@ConditionalOnMissingBean`, etc.) listed in `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`.

### 3. `@ComponentScan`
Tells Spring to scan the current package and all sub-packages for components — classes annotated with `@Component`, `@Service`, `@Repository`, `@Controller`, `@RestController`, etc. — and register them as beans.
...
Placing this class at the root package (e.g., `com.example.myapp`) ensures component scanning covers all sub-packages like `com.example.myapp.controller`, `com.example.myapp.service`, etc. If you put it too deep in the package structure, classes outside its scan scope won't get picked up.

## Useful attributes

You can customize behavior via attributes:

```java
@SpringBootApplication(
    exclude = { DataSourceAutoConfiguration.class },  // disable specific auto-config
    scanBasePackages = "com.example.other"             // scan custom packages
)
```

- `exclude` / `excludeName` — turn off specific auto-configuration classes you don't want.
- `scanBasePackages` / `scanBasePackageClasses` — override the default component scan location.

## Why it matters

Without `@SpringBootApplication`, you'd need to manually write:
...
The single annotation is purely for convenience and readability — functionally identical, but much cleaner.
